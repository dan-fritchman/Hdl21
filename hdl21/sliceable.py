...
def sliceable(cls: Type[T]) -> Type[T]:
    ...
    if getattr(cls, "__getitem__", None) is not None:
        msg = f"Internal hdl21 Error: invalid `slices`-decoration of {cls} with existing __getitem__"
        raise RuntimeError(msg)

    if not is_connectable(cls):
        msg = f"Internal hdl21 Error: invalid `slices`-decoration of {cls} which is not `connectable`"
        raise RuntimeError(msg)

    def __getitem__(self, index: Union[int, slice]) -> "Slice":
        return _slice(parent=self, index=index)

    def __setitem__(self, index: Union[int, slice], val) -> None:
        from .slice import Slice

        if not isinstance(index, (int, slice)):
            raise TypeError

        self._slices.add(Slice(parent=val, index=index))
